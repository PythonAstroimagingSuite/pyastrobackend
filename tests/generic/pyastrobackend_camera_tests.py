...
#    FORMAT = '%(asctime)s %(levelname)-8s %(message)s'
    ...
    logging.basicConfig(filename='pyastrobackend_ccd_tests.log',
                        filemode='w',
                        level=logging.DEBUG,
                        format=FORMAT,
                        datefmt='%Y-%m-%d %H:%M:%S')
    ...
    logging.info(f'pyastrobackend_ccd_tests starting')
    ...
    fileloc = os.path.join(os.getcwd(), 'pyastrobackend_ccd_tests.fits')

    logging.info(f'Writing image data to {fileloc}')

    # test if save image works
    logging.info(f'Camera reports supports_saveimge = {camera.supports_saveimage()}')
    if camera.supports_saveimage():
